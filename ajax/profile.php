<?php
    ...
    if (isset($_POST['info_form'])) {
        ...
        $u_exit = select("SELECT * FROM `user_cred` WHERE `phonenum`=? AND `id`!=? LIMIT 1",
        ...
        $query = "UPDATE `user_cred` SET `name`=?,`address`=?,`phonenum`=?,`pincode`=?,
        `dob`=? WHERE `id`=?";
    ...
    if (isset($_POST['profile_form'])) {
        ...
        $u_exit = select("SELECT `profile` FROM `user_cred` WHERE `id`=? LIMIT 1", [$_SESSION['uId']], 's');
